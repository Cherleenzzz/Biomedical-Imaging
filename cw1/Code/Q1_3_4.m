...
startx = [3.5e+0 3e-03 2.5e-01 2.5e-01 0 -5 0 0];
...
startx(4) = startx(4) +  (startx(4)*random4);
startx(5) = startx(5)+random5;
startx(6) = startx(6)+random6;
startx(7) = startx(7)+random7;
startx(8) = startx(8)+random8;
...
NewStartx(4) = -log((1/startx(4))-1);
NewStartx(5) = startx(5);
NewStartx(6) = startx(6);
NewStartx(7) = startx(7);
NewStartx(8) = startx(8);

% Now run the fitting using NewBallStickSSD function
[parameter_hat,RESNORM,EXITFLAG,OUTPUT]=fminunc(@BallTwoStickSSD,NewStartx,h,meas,bvals,grad_dirs);

t2 = cputime - t1;

% Use NewBallStickSSD function to computes predicted signal and the sum of square differences
[sumRes, S_pred] =BallTwoStickSSD(parameter_hat, meas, bvals, grad_dirs);
...
random4 = randn(1,iterations);
random5 = randn(1,iterations)*2*pi;
random6 = randn(1,iterations)*2*pi;
random7 = randn(1,iterations)*2*pi;
random8 = randn(1,iterations)*2*pi;
...
for i=1:iterations
    ...
    NewStartx(4) = startx(4)*(1+random4(i));
    NewStartx(5) = startx(5)+random5(i);
    NewStartx(6) = startx(6)+random6(i);
    NewStartx(7) = startx(7)+random7(i);
    NewStartx(8) = startx(8)+random8(i);
    % Define various options for the non-linear fitting algorithm.
    h=optimset( 'MaxFunEvals',  20000, ...
                'Algorithm' , 'levenberg-marquardt',   ...  
