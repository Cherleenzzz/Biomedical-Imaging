...
startx = [3.5e+0 3e-03 2.5e-01 0 0 5 2];
...
startx(6) = startx(6) + (startx(6)*random6); 
startx(7) = startx(7) +  (startx(7)*random7);
...
[parameter_hat,RESNORM,EXITFLAG,OUTPUT]=fminunc(@DiffusionTensorSSD,NewStartx,h,meas,bvals,grad_dirs);

t2 = cputime - t1;

% Use NewBallStickSSD function to computes predicted signal and the sum of square differences
[sumRes, S_pred] =DiffusionTensorSSD(parameter_hat, meas, bvals, grad_dirs);
...
random6 = randn(1,iterations);
random7 = randn(1,iterations);
...
for i=1:iterations
    ...
    NewStartx(6) = startx(6)*(1+random6(i));
    NewStartx(7) = startx(7)*(1+random7(i));
    % Define various options for the non-linear fitting algorithm.
    h=optimset( 'MaxFunEvals',  20000, ...
                'Algorithm' , 'levenberg-marquardt',   ...  
