...
startx = [3.5e+0 3e-03 2.5e-01 0 0 5];
...
[parameter_hat,RESNORM,EXITFLAG,OUTPUT]=fminunc(@ZeppelinStickTortSSD,NewStartx,h,meas,bvals,grad_dirs);

t2 = cputime - t1;

% Use NewBallStickSSD function to computes predicted signal and the sum of square differences
[sumRes, S_pred] =ZeppelinStickTortSSD(parameter_hat, meas, bvals, grad_dirs);
...
for i=1:iterations
    ...
    NewStartx(6) = startx(6)*(1+random1(i)); 
    % Define various options for the non-linear fitting algorithm.
    h=optimset( 'MaxFunEvals',  20000, ...
                'Algorithm' , 'levenberg-marquardt',   ...  
