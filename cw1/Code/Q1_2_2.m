...
S0_array= zeros(1,iterations_MCMC);
diff_array = zeros(1,iterations_MCMC);
f_array= zeros(1, iterations_MCMC);

for i=1:iterations_MCMC  
    % Now run the fitting                                          
    ...
    % a=(resnorm_old-resnorm_new)/(2*sigmaNoise^2)
    a = exp((init_RESNORM-RESNORM) / (2*40000^2));
 
    if (a > rand)
        S0_array(i) = parameter_hat(1);
        diff_array(i) = parameter_hat(2);
        f_array(i) = parameter_hat(3);
        best_parameter=parameter_hat;
        init_RESNORM=RESNORM;
    else
        S0_array(i) = best_parameter(1);
        diff_array(i) = best_parameter(2);
        f_array(i) = best_parameter(3);
    end
...
stdev_S0 = std(S0_array);
mean_diff = mean(diff_array);
stdev_diff = std(diff_array);
mean_f = mean(f_array);
stdev_f = std(f_array);

% Calculate ranges
S0_range2sig_MCMC = [mean_S0 - 2*stdev_S0, mean_S0 + 2*stdev_S0];
S0_range95_MCMC = [prctile(S0_array,2.5), prctile(S0_array,97.5)];
diff_range2sig_MCMC = [mean_diff - 2*stdev_diff, mean_diff + 2*stdev_diff];
diff_range95_MCMC = [prctile(diff_array,2.5), prctile(diff_array,97.5)];
f_range2sig_MCMC = [mean_f- 2*stdev_f, mean_f + 2*stdev_f];
f_range95_MCMC = [prctile(f_array,2.5), prctile(f_array,97.5)];
...
hist(S0_array, 50);
hold on
plot(S0_range2sig_MCMC, 0.3*[y_bound(2) ,y_bound(2)],'g-x', 'LineWidth', 0.75);
hold on
plot(S0_range95_MCMC, 0.4*[y_bound(2) ,y_bound(2)],'b-x', 'LineWidth', 0.75);
legend('S0 likelihood distribution','2-sigma range', '95 percent range');
figure,
hist(diff_array, 50);
hold on
plot(diff_range2sig_MCMC, 0.4*[y_bound(2) ,y_bound(2)],'g-x', 'LineWidth', 0.75);
hold on
plot(diff_range95_MCMC, 0.5*[y_bound(2) ,y_bound(2)],'b-x', 'LineWidth', 0.75);
legend('diff likelihood distribution','2-sigma range', '95 percent range');
figure,
hist(f_array, 50);
hold on
plot(f_range2sig_MCMC, 0.4*[y_bound(2) ,y_bound(2)],'g-x', 'LineWidth', 0.75);
hold on
plot(f_range95_MCMC, 0.5*[y_bound(2) ,y_bound(2)],'b-x', 'LineWidth', 0.75);
...
% legend('2-sigma-boot range','2-sigma-MCMC range','95 percent-boot range','95 percent-MCMC range');
% figure,
